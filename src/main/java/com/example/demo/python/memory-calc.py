...
def format_bcb_date(date_str):
    # Converte para formato esperado pela API do Bacen (dd/mm/yyyy)
...
def fetch_selic_over(start_date, end_date):
    ...
    # Consulta API do Bacen (Selic Over - código 1178)
    ...
    # Organiza como dicionário {data: taxa}
    taxas = {}
    for item in data:
        dt = datetime.strptime(item['data'], "%d/%m/%Y").strftime("%Y-%m-%d")
        taxa = float(item['valor'].replace(",", ".")) / 100  # converte % para decimal
        taxas[dt] = taxa
    return taxas
...
def calcular_ftms(taxas, data_inicio, data_fim):
    ...
    for dt in datas:
        ...
        if dt > data_fim:
            break
        taxa_diaria = taxas[dt] / 252  # Selic Over diária
...
data_amortizacao = data_liberacao  # se não houver, igual a liberação
